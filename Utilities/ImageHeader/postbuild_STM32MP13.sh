...
case "$(echo "${PYTHON_VERSION%%.*}")" in
  "Python 2")
    #line for python
    echo Postbuild with Python 2 script
    imgtool="${local_script_path}/Python27/Stm32ImageAddHeader.py"
    cmd="python"
    ;;
  "Python 3")
    #line for window executeable
    echo Postbuild with Python 3 script
...
command="${cmd} ${imgtool} ${elf_file_basename}_postbuild.bin ${elf_file_basename}.stm32 -in \"${app_name}\" -hv 2.1 -bt 10 -ep ${formatted_ep_addr}"
${command}
ret=$?
