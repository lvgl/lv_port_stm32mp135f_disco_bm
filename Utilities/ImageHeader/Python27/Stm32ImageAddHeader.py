...
class Stm32Image:
    ...
    def __init__(self, header_major_ver=0, header_minor_ver=0, entry=0, loadaddr=0, _binary_type=0):
        ...
        self.image_name = b'\x00' * 64                             # Image Name
    ...
    def print_header(self):
        ...
        if self.header_minor_ver > 0:
            print("Image Name  : \"%s\"" % self.image_name.decode())
    ...
    def generate(self, image_name, input_file, output_file):
        ...
        if self.header_major_ver == 1:
            ...
            if self.header_minor_ver == 0:
                ...
                field[16] = struct.pack('64s', self.image_name)             # V2.1: Image Name
                field[17] = struct.pack('312s', self.ext_padding0)          # V2.1: Extension PAD
...
def main():
    ...
    parser.add_argument("-in", "--image_name", action="store", help="image name")
    args = parser.parse_args()

    image_name = args.image_name or args.output_file
    ...
    if args.header_version:
        ...
        header_major_ver = int(header_version_split[0], 10)
        header_minor_ver = int(header_version_split[1], 10)
    else:
        header_major_ver = 2
        header_minor_ver = 0
    ...
    ret = stm32im.generate(image_name, args.input_file, args.output_file)
    if ret != 0:
      print("Aborted")
      return ret
